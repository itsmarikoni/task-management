...
- タグ・ラベル機能
- 複数ボード管理(ボードは1つのみ)
- モバイルアプリ化(ネイティブアプリ。ブラウザでのレスポンシブ対応は対象内)
- マルチユーザー・認証機能
...
- PC・スマートフォンの両方での利用を想定し、モダンブラウザ(Google Chrome等)で動作すること。
- Chrome以外のブラウザ(Edge、Firefox等)でも大きく崩れず動作することが望ましい。
- スマートフォン等の小画面ではレイアウトをレスポンシブに切り替え、タッチ操作によるドラッグ&ドロップに対応する。
- ホーム画面への追加が行えるよう、PWA(Progressive Web App)として構成する。
